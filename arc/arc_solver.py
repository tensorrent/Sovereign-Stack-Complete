...
def _call_llm(messages: list[dict], system: str = "",  # only reached after TENT gate
    ...
    """Call LLM with text-only messages. Tries claude then openrouter."""
...
def analyze_task(task: ARCTask) -> str:
    """Stage 1: Ask LLM to extract the transformation rule."""
...
class ARCSolver:
    ...
    def solve(self, task: ARCTask) -> list[ARCPrediction]:
        ...
        if best_score < 1.0 and self._use_bf:
            ...
            try:
                ...
                bf = brute_force_search(task, max_depth=2,
                                        time_limit=20.0,
                                        verbose=self._verbose,
                                        upg_prim_order=_bra_cfg.get("upg_prim_order"))
        ...
        if best_score < 1.0 and _pipeline != "brute_force":
            self._log("Stage 3: Synthesis + refinement loop...")
            current_prog     = ""
            current_failures = []
            ...
            for attempt in range(1 + MAX_REFINEMENTS):
                ...
                try:
                    if attempt == 0:
                        program = synthesize_program(task, rule)
                    else:
                        program = refine_program(task, rule, current_prog,
                                                 current_failures)
                except Exception as e:
        ...
        for i, test_pair in enumerate(task.test):
            ...
            if predicted is None and best_program:
        ...
        self._scroll_record(
            f"arc_done:{task.task_id}:solved={n_predicted}/{len(predictions)}",
            EV_RESONANCE if n_predicted == len(predictions) else EV_MISS,
            3 if n_predicted == len(predictions) else 1)
